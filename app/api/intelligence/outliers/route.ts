...
export async function GET(request:Request){
 const ventureName=new URL(request.url).searchParams.get("venture")||"";
 if(!ventureName) return NextResponse.json({error:"venture is required"},{status:400});
 const venture=await prisma.venture.findFirst({where:{name:ventureName}});
 if(!venture) return NextResponse.json({error:"Venture not found"},{status:404});
 const items=await prisma.intelligenceEvidence.findMany({where:{ventureId:venture.id,activityStatus:"OUTLIER"},orderBy:{createdAt:"desc"},take:50});
 return NextResponse.json({connected:Boolean(process.env.VIDIQ_API_KEY),venture:ventureName,query:ventureQueries[ventureName]||ventureName,count:items.length,items});
}
...
export async function POST(request:Request){
 try{
  ...
  for(const v of body.videos){
   ...
   if(exists){await prisma.intelligenceEvidence.update({where:{id:exists.id},data:{metrics,observableSignals:signals,mediaUrl:v.videoThumbnail??exists.mediaUrl,activityStatus:"OUTLIER"}});updated++;}
  ...
  return NextResponse.json({ok:true,saved,updated,total:body.videos.length});
