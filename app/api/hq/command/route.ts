...
export async function POST(req:Request){
 ...
 if(/create|write|draft|pitch|email|content|creative|script/.test(q))return NextResponse.json({mode:"route",title:"Ready to create",summary:"I can send this instruction into Create with the business and objective already attached.",items:[],action:{label:"Open Create",target:"Create",handoff:{venture:ventureName,project:command,output:command}}});
 ...
 if(tasks.length||actions.length)return NextResponse.json({mode:"data",title:"What needs attention",summary:"These are the current live HQ tasks and actions for this business.",items:[...actions.map(x=>({id:x.id,title:x.title,meta:x.lane.replaceAll("_"," ")})),...tasks.map(x=>({id:x.id,title:x.title,meta:x.status.replaceAll("_"," ")}))]});
