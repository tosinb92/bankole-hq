...
export default function CreativeStudio() {
  ...
  return <>
    <section className="create-hero"><div><p className="eyebrow">CREATE</p><h2>What do you want to make?</h2><p>Choose the business, choose the output and describe the idea. HQ handles the production steps underneath.</p></div></section>
    
    <section className="creative-workspace"><div className="panel create-form">{intelligenceHandoff && <div className="intelligence-handoff"><b>Intelligence hand-off · {intelligenceHandoff.skill}</b><small>Evidence and approved-skill output were carried into this brief.</small><button onClick={() => setIntelligenceHandoff(null)}>Clear hand-off</button></div>}<label>Venture<select value={venture} onChange={e=>{const next=e.target.value;setVenture(next);setAssets([]);setVideo(null);if(creationType==="Video")setType((videoPresets[next]??["Custom Video"])[0])}}>{Object.keys(venturePresets).map(x=><option key={x}>{x}</option>)}</select></label><label>What do you want to make?<select value={creationType} onChange={e=>{const next=e.target.value;setCreationType(next);setVideo(null);setType(next==="Video"?(videoPresets[venture]??["Custom Video"])[0]:(venturePresets[venture]??[next])[0])}}>{creationTypes.map(x=><option key={x}>{x}</option>)}</select></label><details className="advanced-options"><summary>Project details</summary><label>Project / campaign<input value={project} onChange={e=>setProject(e.target.value)}/></label></details><div className="preset-list">{presets.map(p=><button key={p} className={type===p?"selected-preset":""} onClick={()=>{setType(p);if(isVideo)setPlan(planFor(venture,brief))}}>{p}<b>→</b></button>)}</div><label>Brief / source idea / copy / script<textarea value={brief} onChange={e=>setBrief(e.target.value)} rows={7}/></label><details className="advanced-options"><summary>Format options</summary><label>Aspect ratio<select value={ratio} onChange={e=>setRatio(e.target.value as typeof ratio)}><option value="portrait">9:16 portrait</option><option value="square">1:1 square</option><option value="landscape">16:9 landscape</option></select></label></details>{isVideo&&<p className="note">{type.includes("Animate") ? "Animate the latest generated Studio image. Generate an image first if this project has none yet." : "Text-to-video uses the brief; a latest generated Studio image is automatically used as a reference when available."}</p>}<div className="creative-actions simplified-create"><button className="primary create-main-action" onClick={()=>isVideo?startVideo():makeImage("Generate Image")} disabled={!!busy}>{busy ? "Creating…" : isVideo ? "Create video →" : "Create visual →"}</button>{assets.length>0&&!isVideo&&<button onClick={()=>makeImage("Generate Image Variation",true)} disabled={!!busy}>Make another version</button>}</div></div>
