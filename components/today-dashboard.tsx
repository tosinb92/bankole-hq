...
export default function TodayDashboard({openBusiness,openCreate,openIntelligence}:Props){
 ...
 const priorities=data?.queue.filter(x=>["Bubble Leisure","Brilliant AI Automation","FireComplianceUK","Bankole & Associates"].includes(x.business)).slice(0,4)??[];
 const go=(x:Item)=>x.action==="intelligence"?openIntelligence():x.business!=="HQ"&&openBusiness(x.business);
 return <><section className="exec-hero command-hero"><div><p className="eyebrow">TODAY · REVENUE COMMAND CENTRE</p><h2>What moves money today?</h2><p>Four priorities. Live pipeline. Clear next moves.</p></div><button className="primary" onClick={()=>load()}>↻ Refresh</button></section>
