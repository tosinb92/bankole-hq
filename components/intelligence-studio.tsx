...
export default function IntelligenceStudio({ setView }: { setView: (view: string) => void }) {
  ...
  return <>
    ...
    <section className="panel skill-runner"><div className="panel-title"><h2>Research Inbox · capture what you saw</h2><span>ScraperAPI · public links only</span></div><p>Paste a public Instagram Reel/post, Facebook post, YouTube video/Short or TikTok link. Bankole HQ stores the source, publicly exposed metadata and your reason for noticing it.</p><div className="skill-controls"><label>Public social URL<input value={capture.url} onChange={event => setCapture({ ...capture, url: event.target.value })} placeholder="https://www.instagram.com/reel/…"/></label><label>How did you find it?<select value={capture.interaction} onChange={event => setCapture({ ...capture, interaction: event.target.value })}><option value="SAVED">I saved it</option><option value="LIKED">I liked it</option><option value="SAW">I noticed it</option><option value="COMPETITOR">Competitor research</option></select></label><label>Paid or organic?<select value={capture.distribution} onChange={event => setCapture({ ...capture, distribution: event.target.value })}><option value="ORGANIC">Organic content</option><option value="PAID">Paid advertising</option></select></label></div><label>Why did it interest you?<textarea rows={4} value={capture.note} onChange={event => setCapture({ ...capture, note: event.target.value })} placeholder="Example: strong first-second hook, realistic parent reaction, pacing I want for Bubble Leisure."/></label><div className="output-actions"><button className="primary" onClick={captureSocial} disabled={!!busy || !capture.url.trim()}>{busy === "capture" ? "Collecting through ScraperAPI…" : "Capture to Research Inbox"}</button></div><p className="note">This does not read your private Instagram Saved or Likes folders. It collects only the public link you explicitly provide.</p>{notice && <p className="note">{notice}</p>}</section>
    ...
    <section className="panel skill-runner"><div className="panel-title"><h2>Analyse with approved imported skill</h2><span>{selected.length} selected evidence item{selected.length === 1 ? "" : "s"}</span></div><div className="skill-controls"><label>Scope<select value={scope} onChange={event => setScope(event.target.value)}>{["one creative", "one competitor", "several competitors", "campaign", "platform", "selected date range"].map(item => <option key={item}>{item}</option>)}</select></label><label>Approved SKILL.md<select value={skill} onChange={event => setSkill(event.target.value)} disabled={!approvedSkills.length}>{approvedSkills.length ? approvedSkills.map(item => <option key={item.id}>{item.name}</option>) : <option>Import and approve a skill first</option>}</select></label><button className="primary" onClick={runSkill} disabled={busy === "skill" || !selected.length || !approvedSkills.length}>{busy === "skill" ? "Running approved skill…" : "Run Skill"}</button><button onClick={sendToAskHQ} disabled={!selected.length}>Ask HQ to build workflow →</button></div><p className="note">{approvedSkills.length ? "The application reads actual stored SKILL.md instructions only. It does not create a generic replacement methodology." : "Go to Skills Library → Import actual Drive skills → approve the workflow you want to run."}</p></section>
